...
class TestClientScenarios:
    ...
    async def test_cli_deploy_scenario(self, project_structure):
        ...
        # Mock para SSH e FTP
        with patch(
            "src.deployers.ssh_deployer.asyncssh.connect"
        ) as mock_ssh_connect, patch("aioftp.Client") as mock_ftp_client, patch(
            "src.deployers.ssh_deployer.SSHDeployer.disconnect"
        ) as mock_ssh_disconnect:
    ...
    async def test_error_handling_scenario(self, project_structure):
        ...
        with patch("src.deployers.ssh_deployer.SSHDeployer.connect") as mock_connect:
            ...
            assert str(exc_info.value) == "SSH connection failed"
            mock_connect.assert_called_once()

    @pytest.mark.asyncio
    async def test_client_basic_workflow(self):
        """Testa fluxo básico do cliente"""
        client = DeployClient()
        config_path = Path("tests/fixtures/config.yml")

        # Setup
        await client.setup(config_path)
        assert client.config, "Config should be loaded"
